...
class KeyCoder {
public:
    static std::string Encode(std::string const& row, std::string const& col, uint64_t timestamp) {
        std::string buffer;
    ...
    static std::string PartialEncode(std::string const& row, std::string const& col) {
        std::string buffer;
        buffer.reserve(row.size() + col.size() + 16);
    ...
    static StatusOr<DecodeResult> Decode(std::string const& full_key) {
        ...
        if (full_key.size() - pos != 16) return InternalError("Received invalid key", GCP_ERROR_INFO().WithMetadata("key", full_key));
        ...
        try {
            ...
            return InternalError("Failed to parse timestamp hex", GCP_ERROR_INFO().WithMetadata("ts_hex", ts_hex));
        }
...
private:
    ...
    static StatusOr<std::string> ConsumeField(const std::string& src, size_t& pos) {
        std::string res;
        while (pos < src.size()) {
            size_t zero_idx = src.find('\0', pos);
            if (zero_idx == std::string::npos) {
                return InternalError("Failed to decode key", GCP_ERROR_INFO().WithMetadata("key_left", src));
            }

            res.append(src, pos, zero_idx - pos);
            if (zero_idx + 1 >= src.size()) return InternalError("Failed to decode key", GCP_ERROR_INFO().WithMetadata("key_left", src));
        ...
        return InternalError("Broken key: loop fell through", GCP_ERROR_INFO().WithMetadata("key_left", src));
    }
};
