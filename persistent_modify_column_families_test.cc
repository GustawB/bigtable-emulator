...
TEST_F(PersistentModifyColumnFamiliesTest, SchemaPersistsAcrossReload) {
  std::vector<std::string> initial_cfs = {"fam_initial"};
  auto maybe_table = CreatePersistentTable(initial_cfs);
  ASSERT_STATUS_OK(maybe_table);
  auto table = maybe_table.value();
  ...
  auto maybe_loaded = Table::Load(kTableName, kDataRoot);
...
TEST_F(PersistentModifyColumnFamiliesTest, DataRestoredAfterModifyAndReload) {
  ...
  ASSERT_STATUS_OK(SetCells(table, kTableName, "row1", cells));
  ...
  ASSERT_STATUS_OK(SetCells(table, kTableName, "row2", cells));
  ...
  ASSERT_STATUS_OK(HasPersistentCell(loaded_table, "fam_a", "row1", "col1", 1000, "value_a"));
  ASSERT_STATUS_OK(HasPersistentCell(loaded_table, "fam_b", "row1", "col1", 2000, "value_b"));
  ASSERT_STATUS_OK(HasPersistentCell(loaded_table, "fam_c", "row2", "col1", 3000, "value_c"));
}

TEST_F(PersistentModifyColumnFamiliesTest, DroppedColumnFamilyDataRemoved) {
  std::vector<std::string> initial_cfs = {"fam_a", "fam_b"};
  auto maybe_table = CreatePersistentTable(initial_cfs);
  ASSERT_STATUS_OK(maybe_table);
  auto table = maybe_table.value();
  ...
  ASSERT_STATUS_OK(HasPersistentCell(loaded_table, "fam_a", "row1", "col1", 1000, "value_a"));
  auto utils =
      std::static_pointer_cast<PersistentTableOperations>(loaded_table->GetUtilities());
